...
jsSID.ReSID.EnvelopeGenerator.rate_counter_period = new Uint16Array([
	9, 32, 63, 95, 149, 220, 267, 313, 392, 977, 1954, 3126, 3907, 11720, 19532, 31251
]);

// this one seems like overkill... idx +  (idx<<4) should do it...
jsSID.ReSID.EnvelopeGenerator.sustain_level = new Uint8Array([
...
jsSID.ReSID.WaveformGenerator.prototype.reset = function() {
	...
	this.ignore_waveform = false; //javi
};
...
jsSID.ReSID.Voice = function() {
	...
	this.sample = new Int16Array( jsSID.ReSID.const.RINGSIZE );
	this.sample_last_pos = 0;
};
...
jsSID.ReSID.Voice.prototype.output = function() {
	var sample = 0;
	if (!this.muted)
		sample = this.last_sample = (this.wave.output() - this.wave_zero) * this.envelope.output() + this.voice_DC;
	this.sample[ this.sample_last_pos % this.sample.length ] = sample;
	this.sample_last_pos += 1;
	return sample;
};
...
jsSID.ReSID.Filter = function() {
	this.ignore_commands = false; //javi
...
jsSID.ReSID.prototype.poke = function(offset, value, force) {
	if(this.ignore_commands && !force)
		return;
...
jsSID.ReSID.prototype.set_sampling_parameters = function(clock_freq, method, sample_freq, pass_freq, filter_scale) {
	...
	if (!this.sample) {
		this.sample = new Int16Array(jsSID.ReSID.const.RINGSIZE * 2);
	}
	// Clear sample buffer.
...
jsSID.ReSID.prototype.generate = function(samples) {
        var data = new Int16Array(samples);
